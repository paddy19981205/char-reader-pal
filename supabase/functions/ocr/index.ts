...
【注音符號辨識】
- 如果某個字的旁邊（通常是右側）有注音符號標記，請在該字後方以括號標注完整注音
- 格式：字（注音），例如：學（ㄒㄩㄝˊ）、花（ㄏㄨㄚ）
- 注音符號寫在方格外側，字體較小，請仔細辨識每個注音符號和聲調
- 如果該字沒有標注注音，則正常輸出該字即可，不要自行添加注音
...
- 不要移除或修改標題（如「學習像種樹」）
- 不要改變空行的位置
- 不要刪除、修改或移動任何注音標記（如「學（ㄒㄩㄝˊ）」），必須完整保留
