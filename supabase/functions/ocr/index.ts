...
- 學生可能在某些字的旁邊（通常是右側）標注注音符號（ㄅㄆㄇㄈㄉㄊㄋㄌㄍㄎㄏㄐㄑㄒㄓㄔㄕㄖㄗㄘㄙㄧㄨㄩㄚㄛㄜㄝㄞㄟㄠㄡㄢㄣㄤㄥㄦ）及聲調符號（ˊˇˋ˙）
...
【注音符號辨識 — 嚴格規則】
- 注音符號的特徵：寫在方格「外側」（通常是字的右側邊緣或格線旁），字體明顯比方格內的國字小很多，通常是直排的一串注音符號
- 只有同時滿足以下所有條件才能判定為注音標記：
  1. 符號寫在方格外側（不是寫在方格內部）
  2. 字體大小明顯小於方格內的國字
  3. 由注音符號（ㄅ-ㄦ）和聲調符號（ˊˇˋ˙）組成
- 方格內的國字絕對不要誤判為注音，即使該字的部件看起來像注音符號
- 輸出格式：使用 {注音|ㄓㄨˋㄧㄣ} 標記，例如：{學|ㄒㄩㄝˊ}、{花|ㄏㄨㄚ}
- 如果無法確定是否為注音標記，寧可不標注，避免誤判
...
- 不要移除或修改標題（如「學習像種樹」）
- 不要改變空行的位置
- 不要刪除、修改或移動任何注音標記（如「{學|ㄒㄩㄝˊ}」），必須完整保留格式
